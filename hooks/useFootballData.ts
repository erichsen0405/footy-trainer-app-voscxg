  ...
  const fetchTasks = useCallback(async () => {
    try {
      ...
      const transformed: Task[] = (data || []).map((t: any) => {
        ...
        const mediaNamePayload = buildTaskMediaNamePayload(t.media_names, videoPayload.videoUrls);
        return {
          id: t.id,
          ...
          focusAreas: Array.isArray(t.focus_areas) ? t.focus_areas : [],
          focus_areas: Array.isArray(t.focus_areas) ? t.focus_areas : [],
    ...
    async (task: Omit<Task, 'id'>, options?: AddTaskOptions) => {
      try {
        ...
        const created = await taskService.createTask({
          ...
          mediaNames: task.mediaNames,
          media_names: task.media_names,
          focusAreas: task.focusAreas ?? task.focus_areas ?? [],
          focus_areas: task.focusAreas ?? task.focus_areas ?? [],
  ...
  const updateTask = useCallback(async (id: string, updates: Partial<Task>) => {
    try {
      ...
      await taskService.updateTask(id, session.user.id, {
        ...
        mediaNames: updates.mediaNames,
        media_names: updates.media_names,
    ...
    async (id: string) => {
      try {
        ...
        const copyPayload = {
          ...
          mediaNames: taskToDuplicate.mediaNames,
          media_names: taskToDuplicate.media_names,
          focusAreas: taskToDuplicate.focusAreas ?? taskToDuplicate.focus_areas ?? [],
          focus_areas: taskToDuplicate.focusAreas ?? taskToDuplicate.focus_areas ?? [],
