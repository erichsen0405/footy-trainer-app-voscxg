...
begin
  ...
  if new.title is not distinct from old.title
     ...
     and new.activity_time is not distinct from old.activity_time
     and new.activity_end_time is not distinct from old.activity_end_time
     and new.location is not distinct from old.location
     and new.category_id is not distinct from old.category_id
     and coalesce(new.intensity_enabled, false) is not distinct from coalesce(old.intensity_enabled, false)
  then
    return new;
  end if;
...
begin
  ...
  if new.category_id is not distinct from old.category_id
     and new.local_title_override is not distinct from old.local_title_override
     and coalesce(new.intensity_enabled, false) is not distinct from coalesce(old.intensity_enabled, false)
  then
    return new;
  end if;
