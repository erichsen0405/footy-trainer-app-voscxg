alter table if exists public.task_templates
  add column if not exists auto_add_to_activities boolean;

alter table if exists public.task_templates
  alter column auto_add_to_activities set default false;

update public.task_templates
   set auto_add_to_activities = false
 where auto_add_to_activities is null;

update public.task_templates tt
   set auto_add_to_activities = true
 where exists (
   select 1
...
alter table if exists public.task_templates
  alter column auto_add_to_activities set not null;

create index if not exists task_templates_auto_add_to_activities_idx
  on public.task_templates (user_id, auto_add_to_activities)
  where auto_add_to_activities is true;
