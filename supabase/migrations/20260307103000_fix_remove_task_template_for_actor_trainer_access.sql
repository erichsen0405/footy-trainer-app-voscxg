...
declare
  ...
  v_deleted integer := 0;
  v_effective_exercise_id uuid := null;
begin
  ...
  v_authorized :=
    ...
    or (
      v_template.user_id is not null
      and exists (
        select 1
        from public.admin_player_relationships apr
        where apr.player_id = v_template.user_id
          and apr.admin_id = v_actor
      )
    )
    or (
  ...
  v_effective_exercise_id := v_template.library_exercise_id;

  if v_effective_exercise_id is null then
    select el.id
    into v_effective_exercise_id
    from public.exercise_library el
    where el.trainer_id = v_template.user_id
      and coalesce(el.title, '') = coalesce(v_template.title, '')
      and coalesce(el.description, '') = coalesce(v_template.description, '')
      and coalesce(el.video_url, '') = coalesce(v_template.video_url, '')
    order by el.created_at desc nulls last
    limit 1;
  end if;

  if v_effective_exercise_id is not null then
    delete from public.exercise_assignments ea
    where ea.exercise_id = v_effective_exercise_id
