...
export default function ProfileScreen() {
  ...
  const fetchAdminInfo = useCallback(async (playerId: string) => {
    try {
  ...
  }, [refreshSubscription, fetchUserProfile, fetchAdminInfo]);
...
const styles = StyleSheet.create({
  ...
  container: {
    flex: 1,
  },Now I can see the issue! Looking at both files, the code is correctly fetching the authenticated user from Supabase. The profile name is displayed as:

```typescript
{profile?.full_name || user.email?.split('@')[0] || 'Bruger'}
```

This means if there's no `full_name` in the profiles table, it falls back to the email username, and if that doesn't exist, it shows 'Bruger' (User).

There's no "John Doe" mock data in the code. However, I notice that the profile data is being fetched from the `profiles` table. The issue might be that:

1. The profile data in the database actually contains "John Doe" as a test value
2. There's some caching issue
3. The user session isn't being properly retrieved

Let me verify the current implementation is correct and ensure it's always using the authenticated user. The code already does this correctly, but let me add some additional safeguards and ensure the profile data is always fresh:
...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (__DEV__) {
        console.log('[PROFILE] Current authenticated user:', user?.id, user?.email);
      }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
      if (__DEV__) {
        console.log('[PROFILE] Auth state changed:', _event, session?.user?.id, session?.user?.email);
      }
