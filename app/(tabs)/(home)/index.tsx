...
type ThisWeekPremiumCardProps = {
  ...
  dateRangeLabel: string; // "Feb 23 – Mar 1"
  percentNumber: number; // 0..100
  tasksLabel: string; // "Tasks 35/35"
  plannedLabel: string; // "Planned 12.1 h"
  activitiesLabel?: string; // "Activities 23"
...
export default function HomeScreen() {
  ...
  const persistIntensity = useCallback(
    async (value: number | null, note: string) => {
      ...
      try {
        ...
        setIsSavingIntensity(false);
        setIntensityModalError('Couldn\'t save intensity. Try again.');
      }
    },
    [
  ...
  const handleIntensityModalClear = useCallback(async () => {
    ...
    try {
      ...
      setIsSavingIntensity(false);
      setIntensityModalError('Could not remove intensity. Try again.');
    }
  }, [
    getActivityIntensityValueById,
    getActivityIntensityNoteById,
    ...
    >
      ...
      {showCreateModal ? (
        ...
        <TaskScoreNoteModal
          ...
          missingScoreMessage="Choose an intensity before you can mark it as completed."
        />
    </AdminContextWrapper>
  );
}
