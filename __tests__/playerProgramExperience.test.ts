...
describe('issue #306 player program experience', () => {
  it('calculates player-friendly progress from materialized task and activity completion', () => {
    ...
    expect(result.enrollments[0].progress).toEqual({ completedItems: 2, totalItems: 3, percent: 67 });
    expect(result.enrollments[0].items[0]).toMatchObject({ phaseTitle: 'Foundation', weekNumber: 1 });
    expect(result.enrollments[0].items.map((item: any) => item.status)).toEqual(['completed', 'completed', 'today']);
    expect(result.nextAction).toMatchObject({ enrollmentId: 'enrollment-1', id: 'item-3', status: 'today' });
  });
